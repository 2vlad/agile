...
def get_system_prompt(doc_titles: list[str]) -> str:
    ...
    return (
        ...
        "- Если фрагменты не покрывают вопрос напрямую — всё равно дай полный экспертный ответ. "
        "Ты эксперт, у тебя есть знания за пределами фрагментов. "
        "Кратко отметь что в материалах эта тема напрямую не раскрыта, "
        "но дай исчерпывающий ответ из профессионального контекста, связав с тем, что есть в материалах.\n"
        "- Никогда не описывай содержимое фрагментов. Не пиши «фрагменты посвящены...», "
        "«в найденных фрагментах...». Отвечай на вопрос, а не анализируй фрагменты.\n\n"

        "Формат ответа:\n"
        "1. Начни с прямого ответа — тезис в 1-2 предложениях жирным. Сразу отвечай на вопрос.\n"
        ...
        "- Никаких дисклеймеров, оговорок, извинений.\n"
